...
def packetize(inputlocation,outputlocation):
    ...
    postamble_arr = np.random.randint(2, 1000, size=25600, dtype=np.int32)
...
def retransmit(request_list, inputlocation, outputlocation):
    ...
    if request_list[0] != 'manifest':
        ...
        packet_arr = bytearray()
        preamble_arr = np.random.randint(2, 1000, size=25600, dtype=np.int32)
        preamble = bytes(preamble_arr)
        packet_arr.extend(preamble)
...
def depacketize(filelocation, outputlocation):
    ...
    if not os.path.isfile(pathlib.PurePath(filefolder,'manifest')):
            man_list = []
            man_len_list = []
    ...
    for i in range(1,file_chunk_num + 1):
        if not os.path.isfile(pathlib.PurePath(filefolder, '{filename}_{chunk_num}{file_ext}'.format(filename = filename, chunk_num = i, file_ext = file_ext))):
