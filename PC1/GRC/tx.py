...
class tx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, freq=908e6, samp_rate=2e6, transmit_divider=1):
...
def argument_parser():
    ...
    parser.add_argument(
        "--transmit-divider", dest="transmit_divider", type=eng_float, default=eng_notation.num_to_str(float(1)),
...
def main(top_block_cls=tx, options=None):
    ...
    tb = top_block_cls(freq=options.freq, transmit_divider=options.transmit_divider)
