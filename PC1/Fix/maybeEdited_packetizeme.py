...
def packetize(inputlocation,outputlocation):
    ...
    # input location is the file you want to send, output location is where you want packet.ddi to be created.
    ...
    preamble_arr = np.random.randint(2, 1000)
    ...
    while True:
        if os.path.isfile(pathlib.PurePath(inputfolder, '{filename}_{chunk_num}{file_ext}'.format(filename = filename, chunk_num = i, file_ext = file_ext))):
            ...
            print('Could not open input file location.')
            break
...
def cleanup(fileLocation,which,fileName): # This is run to clean up the folder when the whole packet has been received
    print("Cleaning up folder...")
    if which:
        return fileName
    elif which:
        merger= Merge(fileLocation, fileLocation,fileName)
    ...
    with open(filepath, mode = 'rb') as packet:
        ...
        if packetarray.find('Packet Start'.encode()) != -1:
            while packetarray.find('Packet Start'.encode()) != -1:
                ...
                packetarray = packetarray[endbytes + len('Manifest End'.encode())::]
                man_list.append(manifestbytes)
                man_len_list.append(len(manifestbytes))

            vote_count = Counter(man_len_list)
            winner = vote_count.most_common(1)[0][0]
            winning_manifest = man_list[man_len_list.index(winner)]
            with open(pathlib.PurePath(filefolder,'manifest'), mode = 'wb') as manifest_to_write:
                manifest_to_write.write(winning_manifest)
