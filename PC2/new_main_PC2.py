...
class Control():
    ...
    def setRadio(self, radioType):
        ...
        if radioType == "TX":
            ...
            self.currentServer=subprocess.Popen('python rd2zmq.py', stdout=subprocess.PIPE, 
                shell=True, preexec_fn=os.setsid)
        if _debug:
            print("CONNECTING...")
    ...
    def scanRadio(freq, init):
        if init:
            if freq == "LOW":
                ...
                freq="LOW"
        ct.selectFreq(False,freq)
        if _debug:
            print("Locking")
        ct.currentControl.lock()
        if _debug:
            print("Changing frequency")
        ct.changeFreq()
        ct.changeAmp()
        ct.currentControl.unlock()
        ct.scan(freq)
        if init:
            window.after(10000,scanRadio(freq,1))
            if _debug:
                print(ct.dictTable)
